...
/* ST7789 DDRAM is 240x320 while this panel exposes a 240x280 area.
 * Once MV is set for landscape, the 40-row hidden strip becomes a 20-pixel
 * x offset at each end of the logical landscape address space. */
...
/* These are the only four landscape MADCTL candidates. No canvas, pixel, or
 * font function applies a second coordinate transform. */
static const landscape_profile_t LANDSCAPE_PROFILES[] = {
    {.mirror_x = false, .mirror_y = false, .madctl = 0x20},
    {.mirror_x = true,  .mirror_y = false, .madctl = 0x60},
    {.mirror_x = false, .mirror_y = true,  .madctl = 0xA0},
    {.mirror_x = true,  .mirror_y = true,  .madctl = 0xE0},
};

esp_err_t lcd_st7789_set_orientation(lcd_st7789_t *lcd, lcd_orientation_t orientation)
{
    if (lcd == NULL || lcd->panel == NULL || orientation >= (sizeof(LANDSCAPE_PROFILES) / sizeof(LANDSCAPE_PROFILES[0]))) {
        return ESP_ERR_INVALID_ARG;
    }

    const landscape_profile_t *config = &LANDSCAPE_PROFILES[orientation];
    ESP_RETURN_ON_ERROR(esp_lcd_panel_swap_xy(lcd->panel, true), TAG, "set MV failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_mirror(lcd->panel, config->mirror_x, config->mirror_y), TAG, "set MX/MY failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_set_gap(lcd->panel, LCD_LANDSCAPE_X_GAP, LCD_LANDSCAPE_Y_GAP), TAG, "set panel gap failed");

    ESP_LOGI(TAG, "landscape profile=%u MADCTL=0x%02X gap=(%d,%d)",
             orientation, config->madctl, LCD_LANDSCAPE_X_GAP, LCD_LANDSCAPE_Y_GAP);
    return ESP_OK;
}
...
esp_err_t lcd_st7789_init(lcd_st7789_t *lcd)
{
    ...
    *lcd = (lcd_st7789_t) {0};
    ...
    ESP_RETURN_ON_ERROR(lcd_st7789_set_orientation(lcd, LCD_ORIENTATION_LANDSCAPE_0), TAG, "set initial landscape profile failed");
