...
#### 2. 致命痛点：为什么绝对不能让 CPU 自己去开关灯？

如果按照上面的方法让 CPU 亲自在 `while(1)` 里每毫秒开关一次引脚：
* ❌ **CPU 会被彻底累死**：单片机大脑 100% 的算力全被“手动开关灯”霸占了，根本没空再去连 Wi-Fi、处理触摸屏或读取传感器；
* ❌ **极其容易卡顿**：一旦后台来了一条网络消息打断了 CPU，开关灯的时间稍微延误了几毫秒，灯光就会立刻出现明显的**抽搐和频闪**。
