...
esp_err_t lcd_st7789_init_panel(lcd_st7789_t *lcd)
{
    ...
    *lcd = (lcd_st7789_t) {0};
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .bits_per_pixel = 16,
        /* ESP32 stores uint16_t RGB565 pixels little-endian in memory. */
...
{
    ESP_RETURN_ON_ERROR(lcd_st7789_init_panel(lcd), TAG, "initialize panel failed");
    lcd->framebuffer = heap_caps_malloc(LCD_WIDTH * LCD_HEIGHT * sizeof(uint16_t), MALLOC_CAP_DMA);
    return lcd->framebuffer == NULL ? ESP_ERR_NO_MEM : ESP_OK;
}
...
{
    if (lcd == NULL || lcd->panel == NULL || lcd->framebuffer == NULL || width <= 0 || height <= 0) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    /* RGB565 source data is contiguous only within a row. Flushing one row at
     * a time keeps this function correct for every rectangular region. */
    for (int row = y_start; row < y_end; ++row) {
        esp_err_t result = esp_lcd_panel_draw_bitmap(lcd->panel, x_start, row, x_end, row + 1,
                                                     &lcd->framebuffer[row * LCD_WIDTH + x_start]);
        if (result != ESP_OK) {
            return result;
        }
    }
