...
static void scan_touch_i2c_bus(void)
{
    ...
    ESP_LOGI(TAG, "Touch I2C scan: SCL=GPIO%d SDA=GPIO%d INT=GPIO%d, 400 kHz",
             TOUCH_I2C_SCL_GPIO, TOUCH_I2C_SDA_GPIO, TOUCH_INT_GPIO);

    uint8_t device_count = 0;
    for (uint8_t address = 0x03; address < 0x78; ++address) {
        if (i2c_master_probe(g_touch_i2c_bus, address, 20) == ESP_OK) {
            ESP_LOGI(TAG, "Touch I2C device acknowledged at 0x%02X", address);
            ++device_count;
        }
    }

    if (device_count == 0) {
        ESP_LOGW(TAG, "No device acknowledged. Check the touch flex cable and board power.");
    } else {
        ESP_LOGI(TAG, "Touch I2C scan completed: %u device(s) found", device_count);
    }
}
...
static void build_heart_screen(void)
{
    ...
    g_action_label = create_text(screen, "SW3  START MEASUREMENT", 48, 226,
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "LVGL direct DMA: PSRAM total=%u free=%u, draw buffers=%u bytes each",
    ...
    BaseType_t task_created = xTaskCreate(lvgl_task, "lvgl_ui", 6144, NULL, 5, NULL);
