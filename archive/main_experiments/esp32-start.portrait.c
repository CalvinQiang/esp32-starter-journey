...
static void draw_rect(uint16_t *framebuffer, int x, int y, int width, int height, uint16_t color)
{
    ...
    for (int row = y_start; row < y_end; ++row) {
        for (int column = x_start; column < x_end; ++column) {
            framebuffer[row * LCD_H_RES + column] = color;
        }
    }
}
...
void app_main(void)
{
    ...
    draw_rect(framebuffer, 20, 72, 200, 3, COLOR_ACCENT);
    draw_text_centered(framebuffer, "Hello Calvin", 118, 4, COLOR_WHITE);
    draw_rect(framebuffer, 20, 205, 200, 3, COLOR_ACCENT);
