...
> • **第 00 章 ～ 第 07 章**：✅ **已正式完结**（包含详尽物理原理、小白生活比喻、API 字典与多任务实战）。  
> • **第 08 章 ～ 第 18 章**：🚧 **规划大纲阶段**（目前仅列出课程大纲与源码，正式深度图文教程正在全力连载更新中，敬请期待！）。
...
### 阶段三：传感器集结与通信总线（模拟量与数据通信 · 连载中 🚧）
- [x] **第 07 关：ESP32 模拟量采集(ADC 测温)与超声波测距(HC-SR04)** 📖 [【阅读深度教程】](./book/07_ADC模数转换与超声波测距.md) *(温声动态补偿雷达)*
- [ ] **第 08 关：I2C 通信总线探秘(I2C Scanner)与 DHT11 单总线时序解析** ⏳ *[大纲预告]*

### 阶段四：本地存储与视觉交互（现代化人机界面 · 筹备中 🚧）
- [ ] **第 09 关：ESP32 NVS 非易失性存储与 Flash 偏好设置(断电不丢数据)** ⏳ *[大纲预告]*
- [ ] **第 10 关：ESP32 驱动 1.69寸 ST7789 彩屏与几何图形渲染(SPI DMA)** ⏳ *[大纲预告]*
- [ ] **第 11 关：ESP32 搭载 LVGL v9 现代图形界面与 CST816S 电容触摸实战** ⏳ *[大纲预告]*

### 阶段五：无线互联与智能物联网（打通手机与云端 · 筹备中 🚧）
- [ ] **第 12 关：ESP32 Wi-Fi 联网、SNTP 网络授时时钟与 HTTP/cJSON 天气获取** ⏳ *[大纲预告]*
- [ ] **第 13 关：ESP32 MQTT 物联网双向通信与云平台联动实战(手机远程控制)** ⏳ *[大纲预告]*
- [ ] **第 14 关：ESP32 BLE 低功耗蓝牙实战与微信小程序双向互联** ⏳ *[大纲预告]*

### 阶段六：架构、扩展与毕业设计实战（进阶拓展 · 筹备中 🚧）
- [ ] **第 15 关：ESP32 挂载 MicroSD/TF 卡(4-bit SDIO)与 FATFS 电子相册** ⏳ *[大纲预告]*
- [ ] **第 16 关：ESP32 低功耗电源管理与 Deep-sleep 休眠唤醒(电池省电技术)** ⏳ *[大纲预告]*
- [ ] **第 17 关：嵌入式软件工程化 —— 驱动/业务分层、事件总线与组件化模块设计** ⏳ *[大纲预告]*
- [ ] **第 18 关：ESP32 终极综合大实战 —— 桌面多功能智能气象站与物联网超级中控台 (毕业设计)** ⏳ *[大纲预告]*
...
### 🚧 规划大纲章节（深度图文连载中）
* 📝 [**第 08 章：ESP32 I2C 通信总线探秘与 DHT11 单总线温湿度解析**](./book/08_I2C总线探秘与DHT11温湿度解析.md) *(大纲)*
* 📝 [**第 09 章：ESP32 NVS 非易失性存储与 Flash 偏好设置**](./book/09_NVS非易失性存储与Flash偏好设置.md) *(大纲)*
* 📝 [**第 10 章：ESP32 驱动 1.69寸 ST7789 彩屏与几何图形渲染**](./book/10_ST7789彩屏驱动与几何图形渲染.md) *(大纲)*
* 📝 [**第 11 章：ESP32 搭载 LVGL v9 现代图形界面与电容触摸实战**](./book/11_LVGL图形框架与电容触摸实战.md) *(大纲)*
* 📝 [**第 12 章：ESP32 Wi-Fi 连接管理与 HTTP 互联网天气时钟**](./book/12_WiFi连接管理与HTTP天气时钟.md) *(大纲)*
* 📝 [**第 13 章：ESP32 MQTT 协议接入与阿里云 IoT 实战**](./book/13_MQTT协议接入与阿里云IoT实战.md) *(大纲)*
* 📝 [**第 14 章：ESP32 BLE 低功耗蓝牙与小程序双向互联**](./book/14_BLE低功耗蓝牙与微信小程序互联.md) *(大纲)*
* 📝 [**第 15 章：ESP32 挂载 MicroSD/TF 卡与 FATFS 文件系统**](./book/15_TF卡文件系统与电子相册.md) *(大纲)*
* 📝 [**第 16 章：ESP32 低功耗电源管理与 Deep-sleep 睡眠唤醒**](./book/16_低功耗电源管理与DeepSleep.md) *(大纲)*
* 📝 [**第 17 章：嵌入式软件工程化 —— 驱动/业务分层与事件总线**](./book/17_嵌入式软件工程与模块化分层架构.md) *(大纲)*
* 📝 [**第 18 章：ESP32 终极综合大实战 —— 桌面智能气象站与超级中控台**](./book/18_桌面智能气象站与物联网超级中控.md) *(大纲)*
...
└── 18_final_station_hub/   # Level 18: 实验1(多中枢架构) / 实验2(终极桌面智能中控台全栈总成)
```
...
./switch_code.sh 7 4          # 切换到第 07 关实验 4 (温声融合雷达系统)

# 3. 一键切换并直接编译/烧录运行
./switch_code.sh 7 4 --flash  # 切换并自动烧录启动监视器
```
...
│
├── book/                          # 📖 开源电子书教程（00~07章已完结，08~18章连载中）
│   ├── SUMMARY.md                 # 教程目录大纲
│   ├── 00_环境搭建与开发准备.md
