    ...
    for (int row = 0; row < 7; ++row) {
        for (int column = 0; column < 5; ++column) {
            if ((glyph[row] >> (4 - column)) & 0x01) {
                /* Rotate the 5x7 glyph in place by 180 degrees.  The text
                 * order is intentionally unchanged, so the word still reads
                 * from left to right after the per-glyph correction. */
                const int rotated_column = 4 - column;
                const int rotated_row = 6 - row;
                draw_rect(framebuffer,
                          x + rotated_column * scale,
                          y + rotated_row * scale,
                          scale, scale, color);
            }
        }
